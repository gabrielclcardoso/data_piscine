...
def main():
    ...
    standardize_data(train_data, test_data)
    print_data(train_data, test_data)
...
def standardize_data(train: pd.DataFrame, test: pd.DataFrame) -> None:
...
def plot_train_data(data: pd.DataFrame, ax: plt.Axes) -> None:
    ...
    jedi.plot.scatter(x="Empowered", y="Stims", **jedi_args, **args)
    sith.plot.scatter(x="Empowered", y="Stims", **sith_args, **args)


def plot_test_data(data: pd.DataFrame, ax: plt.Axes) -> None:
    """Plots the scatter plots with no distinction between jedi and Sith"""

    args = {"alpha": 0.5, "s": 30, "c": "green", "label": "Knight", "ax": ax}
